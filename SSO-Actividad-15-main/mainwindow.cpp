...
void MainWindow::showPCB()
{
    ...
    for (size_t a = 0; a < Procesos.size(); ++a)
    {
        aux = Procesos[a];
        ID = new QTableWidgetItem(QString::number(aux.getIdProceso()));
        Estado = (aux.getEjecutado()) ? new QTableWidgetItem("Nuevo"): new QTableWidgetItem("Reanudado");
        Operacion = new QTableWidgetItem(aux.getOperacion());
        Resultado = new QTableWidgetItem("-");
    ...
    //Procesos Nuevos
    for (int a = 0; a < Susp.size(); ++a)
    {
        ID = new QTableWidgetItem(QString::number(Susp[a].Id));
        Estado = new QTableWidgetItem("Suspendido");
        Operacion = new QTableWidgetItem("-");
        Resultado = new QTableWidgetItem("-");
        ...
        TBloq = new QTableWidgetItem("-");
        TRes = new QTableWidgetItem("-");
        Tamanio = new QTableWidgetItem(QString::number(Susp[a].size));
...
void MainWindow::llenarTabla()
{
    while(!Procesos.empty())
    {
        ...
        {
            if(!Procesos.front().getEjecutado())
            {
...
void MainWindow::createFile()
{
    ...
    {
        message.setText("¡Error! No fue posible crear el archivo."
                        "Intenta ejecutar el programa de nuevo.");
        message.setIcon(QMessageBox::Critical);
        message.setWindowTitle("Error!");
        message.setWindowIcon(QIcon(":/images/cancelar.png"));
        message.exec();
...
Proceso MainWindow::readFile()
{
    QFile file("suspendidos.txt");
    if(file.open(QIODevice::ReadWrite | QIODevice::Text))
    {
        QString suspendidos, primero, helper;
        ...
        while(!leer.atEnd())
        {
            if(first)
            {
                primero = leer.readLine();
                first=false;
            }
            else
            {
                QString line = leer.readLine();
                suspendidos.append(line + "\n");
            }
        }
        file.resize(0);
        leer << suspendidos;
        ...
        {
            //Asignar variables leídas a un proceso y mandarlo a memoria.
            if(primero[i]!='|')
            {
                if(primero[i]!='\n')
                {
            ...
            else
            {
                ...
                {
                    case 1:
                        aux3.setIdProceso(helper.toInt());    //ID
                    break;
                    case 2:
                        aux3.setTiempoMaximo(helper.toInt()); //TME
                    break;
                    case 3:
                        aux3.setOperacion(helper);            //Operación
                    break;
                    case 4:
                        aux3.setResultado(helper.toFloat());  //Resultado
                    break;
                    case 5:
                        aux3.setTiempoTranscurrido(helper.toInt()); //Transcurrido
                    break;
                    case 6:
                        aux3.setTRes(helper.toInt());        //Respuesta
                    break;
                    case 7:
                        aux3.setTLlegada(helper.toInt());   //Llegada
                    break;
                    case 8:
                        aux3.setTamanio(helper.toInt());   //Tamanio
                        aux3.setEjecutado(true);
                    break;
                    default:
                    break;
                }
...
void MainWindow::writeFile(Proceso p)
{
    suspendido sus;
    QFile file("suspendidos.txt");
    if (file.open(QFile::Append | QFile::Text))
    {
        QString process;
        process.clear();
        process+= QString::number(p.getIdProceso()) + "|";    //ID
        process+= QString::number(p.getTiempoMaximo()) + "|"; //TME
        process+= p.getOperacion() + "|";                     //OP
        process+= QString::number(p.getResultado()) + "|";    //Resultado
        process+= QString::number(p.getTiempoTranscurrido()) + "|"; //TT
        process+= QString::number(p.getTRes()) + "|";         //Tiempo Respuesta
        process+= QString::number(p.getTLlegada()) + "|";     //T Llegada
        process+= QString::number(p.getTamanio()) + "|\n";      //Tamaño
        sus.Id=p.getIdProceso();
        sus.size = p.getTamanio();
        Susp.push_back(sus);
...
void MainWindow::updateSuspendidos()
{
    if(Susp.empty())
    {
        ui->SuspendidosQLE->setText("-");
        ui->IdSuspendidoQLE->setText("-");
        ui->TamanioSuspendidoQLE->setText("-");
    }
    else
    {
        ui->SuspendidosQLE->setText(QString::number(Susp.size()));
        ui->IdSuspendidoQLE->setText(QString::number(Susp.front().Id));
        ui->TamanioSuspendidoQLE->setText(QString::number(Susp.front().size));
    }
}
...
void MainWindow::keyPressEvent(QKeyEvent *ev)
{
    ...
    else if (ev->key()==Qt::Key_S)
    {
        ...
        {
            if(Bloqueados.front().getTBloqueado()<5)
            {
                aux3 = Bloqueados.front();
                Bloqueados.erase(Bloqueados.begin());
                aux3.setTBloqueado(0);
                writeFile(aux3);
                populateBlocked();
                Pagina p;
                p.setIdProceso(aux3.getIdProceso());
                actualizarTabla(p,"Disponible");
                int marcos = aux3.getTamanio()/5;
                if(aux3.getTamanio()%5>0)
                {
                    marcos+=1;
                }
                paginasDisponibles+=marcos;
    ...
    else if (ev->key()==Qt::Key_R)
    {
        if(Susp.size()>0)
        {
            Proceso reg;
            reg = readFile();
            Susp.erase(Susp.begin());
            updateSuspendidos();
            Procesos.insert(Procesos.begin(),reg);
            updateNext();
            llenarTabla();
        }
    }
...
void MainWindow::showData()
{
    ...
    llenarTabla();
    while(Pendientes.size() + Bloqueados.size() + Susp.size()>0)
    {
        populatePendingTable();
        delay();
        ui->NoProcPendientesQLE->setText(QString::number(Procesos.size()));
        ...
        if(!Pendientes.empty())
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    int marcos = Terminados.back().getTamanio()/5;
                    if(Terminados.back().getTamanio()%5>0)
                    {
                        marcos+=1;
                    }
                    paginasDisponibles+=marcos;
